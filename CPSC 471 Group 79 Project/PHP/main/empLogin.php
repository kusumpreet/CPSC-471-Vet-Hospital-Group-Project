...
<body>
    <nav class="nav">
        ...
        <div class="login-page">
            <form id="loginForm" action="empLogin.php"  method="post">
                <h1 class="loginForm-title">Employee Login</h1>
                ...
                <button class="btn" onclick="location.href='empLogin.php'" type="submit">Continue</button>
            </form>
        </div>
 ...
 else 
 {
    ...
    if (mysqli_connect_errno($con))
      ...
      echo "Failed to connect to MySQL: " . mysqli_connect_error();
        ...
        $result = mysqli_query($con,"SELECT vet_ID FROM vet where username='".$username."' and password='".$password."'");
        if (!$result)
            {
            ...
            if ($num_results == 0)
                ...
                header("refresh:5; url=empLogin.php");
                }
            else
                {
                echo '<br> correct password entered <br/>';
                $row = mysqli_fetch_array($result);
                header("Location:../Employee/getEmployee.php?id=".$row['vet_ID']);
                exit();
                }
            }
        }  
